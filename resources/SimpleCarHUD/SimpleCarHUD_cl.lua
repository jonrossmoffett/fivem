...
local screenPosX = 0.250                    -- X coordinate (top left corner of HUD)
local screenPosY = 0.870                    -- Y coordinate (top left corner of HUD)

-- GENERAL PARAMETERS
local enableController = true               -- Enable controller inputs

-- SPEEDOMETER PARAMETERS
local speedLimit = 110.0                    -- Speed limit for changing speed color
...
-- SEATBELT PARAMETERS
local seatbeltInput = 121                   -- Toggle seatbelt on/off with K or DPAD down (controller)
        ...
        if pedInVeh or locationAlwaysOn then
            ...
            if pedInVeh and GetIsVehicleEngineRunning(vehicle) and vehicleClass ~= 13 then
                ...
                if ShouldUseMetricMeasurements() then
                    ...
                    drawTxt(("%.3d"):format(math.ceil(speed)), 2, speedColor, 0.8, screenPosX + 0.055, screenPosY + 0.000)
                    drawTxt("KPH", 2, speedColorText, 0.4, screenPosX + 0.085, screenPosY + 0.018)
                else
                ...
                drawTxt(("%.3d"):format(math.ceil(currentFuel)), 2, fuelColor, 0.8, screenPosX + 0.000, screenPosY + 0.000)
                drawTxt("LTRS", 2, fuelColorText, 0.4, screenPosX + 0.030, screenPosY + 0.018)
